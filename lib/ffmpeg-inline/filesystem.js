...
function removeFile(instance, fileName) {
  return new Promise(function (resolve, reject) {
    getFile(instance, fileName).then(function (fileInstance) {
      fileInstance.remove(resolve, reject)
    })
  })
}

function removeFiles(instance, fileNames) {
  return new Promise(function (resolve, reject) {
    const removeFilePromises = _.map(fileNames, removeFile.bind(this, instance))
    Promise.all(removeFilePromises).then(function(results) {
      resolve(results)
    })
  })
}
