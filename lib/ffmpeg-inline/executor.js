...
function run(container, args, cbs) {
  return new Promise(function(resolve, reject) {
    ...
    function handleLoadstart(event) {
      return cbs.onLoadstart && cbs.onLoadstart(event)
    }

    function handleProgress(event) {
      return cbs.onProgress && cbs.onProgress(event)
    }

    function handleLoad(event) {
      return cbs.onLoad && cbs.onLoad(event)
    }

    function handleLoadend(event) {
      return cbs.onLoadend && cbs.onLoadend(event)
    }
    ...
    function handleMessage(event) {
      ...
      if (dataIsProgress(data)) {
        ...
        return cbs.onMessage && cbs.onMessage(progress)
      }
    }
    ...
    el.addEventListener('loadend', handleLoadend)
